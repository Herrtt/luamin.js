...
function parseFloat(str, radix) {
  // Thanks stackoverflow (hex numbers with decimal)
  if (!str) return 0;
...
let AllIdentChars = [
  ...
  "x",
  "y",
  "z", // this was actually fucking retarded to add, pls dont do this to me
];
...
let BlockFollowKeyword = ["else", "elseif", "until", "end"];
  ...
  function token(type) {
    ...
    if (type == "Number") {
      if (src.substr(0, 2).toLowerCase() == "0x") {
        ntype = "hex";
        if (parseInt(src, 16) < 999999999999) src = parseInt(src, 16);
...
function CreateLuaParser(text) {
  ...
  function isBlockFollow() {
    ...
    return (
      tok.Type == "Eof" ||
      (tok.Type == "Keyword" && BlockFollowKeyword.includes(tok.Source))
    );
  }
  ...
  function prefixexpr(locals, upvals) {
    ...
    if (tk.Source == "(") {
      ...
      if (
        locals[node.Token.Source] != null &&
        locals[node.Token.Source]?.Tokens?.push != null
      ) {
      ...
      print(debugMark());
      let a = `${getTokenStartPosition(tk)}: Unexpected symbol. ${tk.Type} ${
        tk.Source
      }`;
      throw a;
    }
  ...
  function localdecl(locals, upvals) {
    ...
    if (peek().Source == "function") {
      ...
      if (funcStat.NameChain.length > 1) {
        throw (
          getTokenStartPosition(funcStat.Token_NameChainSeperator[0]) +
          ": `(` expected."
        );
      }
...
function AddVariableInfo(ast) {
  ...
  visitor.FunctionLiteral = {
    Pre: function (expr) {
      ...
      expr.ArgList.forEach((ident, index) => {
        let _var = addLocalVar(
          ident.Source,
          function (name, d) {
            if (!d) ident.Source = name;
          },
          {
            Type: "Argument",
            Index: index,
  ...
  visitor.LocalFunctionStat = {
    Pre: function (stat) {
      addLocalVar(
        stat.FunctionStat.NameChain[0].Source,
        function (name) {
...
function FormatAst(ast) {
  ...
  function applyIndent(token) {
    ...
    if (
      token.LeadingWhite == "" ||
      token.LeadingWhite.substr(-indentString.length, indentString.length) !=
        indentString
    ) {
    ...
    if (expr.Type == "BinopExpr") {
      ...
      if (expr.Type == "HashLiteral") {
        expr.Token.Source =
          '"' +
          hashString(
            `${expr.Token.Source.substring(1, expr.Token.Source.length - 1)}`
          ) +
          '"';
        expr.Type = "StringLiteral";
      ...
      if (expr.EntryList.length == 0) {
        ...
        expr.EntryList.forEach((entry, index) => {
          if (entry.EntryType == "Field") {
            ...
            if (expr.EntryList.length > die) trimToken(entry.Token_OpenBracket);
          ...
          let sep = expr.Token_SeperatorList[index];
          if (sep != null) {
            if (expr.EntryList.length > die) sep.LeadingWhite = "";
            else trimToken(sep);
          }
        });
        undent();
  ...
  formatStat = function (stat) {
    if (stat.Type == "StatList") {
      ...
      if (
        stat.FunctionStat.ArgList.length > 0 &&
        stat.FunctionStat.Token_Varg
      ) {
...
function StripAst(ast) {
  ...
  function joint(tokenA, tokenB, shit = false) {
    ...
    let lastCh = (
      typeof tokenA.Source == "string"
        ? tokenA.Source
        : tokenA.Source.toString()
    ).substr(tokenA.Source.length - 1, 1);
    let firstCh = (
      typeof tokenB.Source == "string"
        ? tokenB.Source
        : tokenB.Source.toString()
    ).substr(0, 1);

    if (
      (lastCh == "-" && firstCh == "-") ||
      (AllIdentChars.includes(lastCh) && AllIdentChars.includes(firstCh)) ||
      (shit && lastCh == ")" && firstCh == "(")
    ) {
  ...
  stripStat = function (stat) {
    if (stat.Type == "StatList") {
      ...
      joint(
        stat.GeneratorList[stat.GeneratorList.length - 1].GetLastToken(),
        stat.Token_Do
      );
      ...
      joint(
        stat.RangeList[stat.RangeList.length - 1].GetLastToken(),
        stat.Token_Do
      );
      bodyjoint(stat.Token_Do, stat.Body, stat.Token_End);
    } else if (stat.Type == "WhileStat") {
...
function SolveMath(ast) {
  // This is some ugly code sorry for whoever is seeing this
  let solveStat;
  let solveExpr;
  ...
  function solvebinop(operator, left1, right1) {
    ...
    if (
      lhs.Type == "VariableExpr" ||
      lhs.Type == "CallExpr" ||
      lhs.Type == "BinopExpr" ||
      rhs.Type == "CallExpr" ||
      rhs.Type == "BinopExpr" ||
      rhs.Type == "VariableExpr"
    ) {
      // some shit later
      // if lhs == true, rhs()
      // if lhs == false, no rhs :(
    ...
    let a =
      lhs.Token || (lhs.Expression != null && lhs.Expression.Token) || null;
    let b =
      rhs.Token || (rhs.Expression != null && rhs.Expression.Token) || null;
    ...
    if (lhs.Type == "StringLiteral" || lhs.Type == "HashLiteral")
      left = lSrc.toString();
    if (rhs.Type == "StringLiteral" || rhs.Type == "HashLiteral")
      right = rSrc.toString();
    ...
    if (left != null && right != null) {
      ...
      if (
        operator == ".." &&
        lhs.Type == "StringLiteral" &&
        rhs.Type == "StringLiteral"
      )
      ...
      if (
        val == false ||
        val == true ||
        (isFinite(val) && val > -(10 ** 6) && val < 10 ** 6)
      )
        return val;
    }
  }
  ...
  function solveunop(operator, rhs) {
    ...
    if (rhs.Type == "TableLiteral" && b != null) {
      ...
      b.forEach((v, i) => {
        if (ignoreRest) {
          ...
          if (v.EntryType == "Value" || v.EntryType == "Index") {
            if (
              (v.Index == null || v.Index.Type == "NumberLiteral") &&
              v.Value
            ) {
              let index =
                v.Index != null &&
                v.Index.Token != null &&
                v.Index.Token.Source !== null
                  ? v.Index.Token.Source
                  : lastIndex + 1;
      ...
      if (operator == "#") {
        ...
        if (rhs.EntryList.length <= 0) {
          return createtype(
            "NumberLiteral",
            amount !== null ? amount : rhs.EntryList.length
          );
        } else if (amount <= 0) {
          return createunop("#", rhs);
        }

        let newex = createbinop(
          "+",
          createtype("NumberLiteral", amount),
          createunop("#", rhs)
        );
        return newex;
      }
    ...
    if (operator == "not" && rhs.Type !== null) {
      if (
        rhs.Type == "NilLiteral" ||
        (rhs.Type == "BooleanLiteral" && right === false)
      )
        return true;
  ...
  solveExpr = function (expr) {
    if (expr.Type == "BinopExpr") {
      ...
      if (expr.Rhs != null && canSolve[expr.Rhs.Type] == true) {
        ...
        if (tokenOp != null && tokenOp.Source != null) {
          let rhs =
            expr.Rhs.Expression != null ? expr.Rhs.Expression : expr.Rhs;
      ...
      if (
        ...
        expr.FunctionArguments.ArgList.forEach((v, i) => {
          ...
          if (
            c !== undefined &&
            (c.Type == "NumberLiteral" ||
              c.Type == "StringLiteral" ||
              c.Type == "NilLiteral" ||
              c.Type == "BooleanLiteral" ||
              c.Type == "HashLiteral")
          ) {
  ...
  solveStat = function (stat) {
    if (stat.Type == "StatList") {
      ...
      if (
        stat.Body.Type == "StatList" &&
        stat.Body.StatementList.length === 0
      ) {
        //return stat.Remove()
      }
    } else if (stat.Type == "GenericForStat") {
      ...
      if (
        ...
        b.Type != "NumberLiteral" ||
        (c != null && c.Type != "NumberLiteral") ||
        c == null
      ) {
        return; // Nope.
      }
      ...
      if (
        stat.Body === null ||
        (stat.Body.Type == "StatList" && stat.Body.StatementList.length === 0)
      ) {
        return stat.Remove();
...
for (i = "a".charCodeAt(); i <= "z".charCodeAt(); i++)
  VarDigits.push(String.fromCharCode(i));
for (i = "A".charCodeAt(); i <= "Z".charCodeAt(); i++)
  VarDigits.push(String.fromCharCode(i));
...
let VarStartDigits = [];
for (i = "a".charCodeAt(); i <= "z".charCodeAt(); i++)
  VarStartDigits.push(String.fromCharCode(i));
for (i = "A".charCodeAt(); i <= "Z".charCodeAt(); i++)
  VarStartDigits.push(String.fromCharCode(i));
        ...
        if (!otherVar.Renamed) {
          if (!otherVar.Scope || otherVar.Scope.Depth < _var.Scope.Depth) {
            otherVar.ReferenceLocationList.some((refAt) => {
              if (
                refAt >= _var.BeginLocation &&
                refAt <= _var.ScopeEndLocation
              ) {
            ...
            _var.ReferenceLocationList.some((refAt) => {
              if (
                refAt >= otherVar.BeginLocation &&
                refAt <= otherVar.ScopeEndLocation
              ) {
                otherVar.UsedNameArray[i] = true;
                return true;
              }
            ...
            if (
              _var.BeginLocation < otherVar.EndLocation &&
              _var.EndLocation > otherVar.BeginLocation
            ) {
              otherVar.UsedNameArray[i] = true;
            }
          }
